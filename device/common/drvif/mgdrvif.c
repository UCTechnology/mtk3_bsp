 ...
 *    Modified by UC Technology at 2012/09/18.
 *    Modified by UC Technology at 2012/11/27.
 ...
 *    Modified by UC Technology at 2012/11/26.
...
#define	NEED_CHKMEM	(0)
#define	NEED_LOCKMEM	(0)
...
typedef struct DeviceRequestQueue {
	...
	UINT		wptn;	/* Event flag pattern for completion notification */
} DRQ;
...
struct GeneralDriverInterface {
	...
	ID		wai_flgid;	/* Event flag for completion notification */
 ...
 */
#define	FREEQ_RD	0x0004		/* Acceptance of "TDC_READ" request is enabled */
#define	FREEQ_WR	0x0040		/* Acceptance of "TDC_WRITE" request is enabled */
...
/*
 * Flag pattern for completion notification
 */
#define DRQWPTN(n) ( (UINT)1 << (n) )
#define DRQWPTN_ALL ( ~((UINT)0) )
...
EXPORT INT GDI_Accept( T_DEVREQ **devreq, INT _acpptn, TMO tmout, GDI gdi )
{
	...
	if ( (_acpptn & DRP_ADSEL) == 0 ) {
		...
		acpptn = ((_acpptn & DRP_NORMREQ) << DRP_ADSEL_OFS) | _acpptn;
	} else {
	...
	for ( ;; ) {
		...
		for ( q = gdi->acpq.next; q != &gdi->acpq; q = q->next ) {
			...
			if ( req->start < 0 ) rptn <<= DRP_ADSEL_OFS;
	...
	if ( (reqptn & DRP_REQMASK) != 0 ) {
		if ( (_acpptn & DRP_ADSEL) == 0 ) {
			/* Normal specification */
			reqptn |= (reqptn & (DRP_NORMREQ << DRP_ADSEL_OFS)) >> DRP_ADSEL_OFS;
			reqptn &= ~(DRP_NORMREQ << DRP_ADSEL_OFS);
		} else {
			/* Extended specification*/
			reqptn |= DRP_ADSEL;
		}
...
EXPORT void GDI_Reply( T_DEVREQ *req, GDI gdi )
{
	...
	if ( drq->wtid > 0 ) {
		...
		tk_set_flg(gdi->wai_flgid, drq->wptn);
	}
	UnlockGDI(gdi);
}
...
#endif	/* NEED_CHKMEM */
#if	NEED_LOCKMEM
		if ( (gdi->def.drvatr & TDA_LOCKREQ) != 0 ) {
...
LOCAL INT gdi_waitfn( T_DEVREQ *devreq, INT nreq, TMO tmout, GDI gdi )
{
	...
	if ( complete == NULL ) {
		/* Wait-for-completion */
		if ( abort == 0 ) {
			err = tk_wai_flg(gdi->wai_flgid, accptn, TWF_ORW,
							 &accptn, tmout);
			if ( err < 0 ) {
				accptn = 0;
			}
		...
		for ( i = 0; i < nreq; ++i ) {
			...
			if ( validDRQ(drq, gdi) ) {
				drq->wtid = 0;
				if ( nreq == 1 && (drq->wptn & accptn) == 0 ) {
					/* Do nothing */
				}
				else if ( complete == NULL && drq->done == Done ) {
					complete = drq;  /* Completed request*/
					n = i;
				}
			} else {
	...
	/* Clear a bit pattern for the corresponding DRQ */
	tk_clr_flg( gdi->wai_flgid, ~complete->wptn );

#if	NEED_LOCKMEM
	/* "memsz > 0" when "buf" space needs to be unlocked */
	memsz = ( (gdi->def.drvatr & TDA_LOCKREQ) != 0 )? complete->memsz: 0;
...
/*
 * Device registration
 *	Device shall be registered in accordance with "ddev" registarion information.
...
EXPORT ER GDefDevice( GDefDev *ddev, T_IDEV *idev, GDI *p_gdi )
{
	...
	if ( ddev->maxreqq < 1 || ddev->maxreqq > GDRVIF_MAX_MAXREQQ ) {
		err = E_PAR; goto err_ret1;
	}
	...
	strncpy((char *)&cflg.exinf, (char *)gdi->def.devnm, sizeof(void *));
	...
	/* Create the event flag for completion notification */
	cflg.exinf   = NULL;
	cflg.flgatr  = TA_TPRI | TA_WMUL;
	cflg.iflgptn = 0;
	err = tk_cre_flg(&cflg);
	if ( err < E_OK ) goto err_ret3a;
	gdi->wai_flgid = (ID)err;
 ...
 *
 * !!ATTENTION!!
 *   This function MUST NOT be called while some jobs are executing since
 *  this function CANNOT abort any jobs.
 */
